...
W_LIMITS = (0.00, .25)
shares = [  # XLV == VHT
    'XLU', 'QQQ', 'SPY', 'GLD','SCHD', 'AAPL', 'GOOG', 'MSFT', 'TSM', "AMD", "NVDA"
...
WEIGHTS = [
    1,     7,    7   ,  9, 7,  5      , 5,    4,    4,   5,    2
]
shares_view = []

DAYS = 360  # day for data analysis
ALPHA = 0.08 
RISK_FREE_ANUL_PERC = 0
...
# %% MAIN
data, w_caps, views = bulk_stocks(shares, shares_view, WEIGHTS)
...
lmb = (mean - RISK_FREE) / var  # Calculate risk aversion
# Calculate equilibrium excess returns (Pi)
Pi = np.dot(np.dot(lmb, cov_returns.copy()), w_caps)

# --- MINIMAL ADD: blend prior with historical mean excess returns ---
 # 0 → trust prior only, 1 → trust historical means only
mu_excess = mean_returns.copy() - RISK_FREE
bl_excess = (1 - ALPHA) * Pi + ALPHA * mu_excess
# -------------------------------------------------------------------

# Use blended BL expected returns (convert back to total return by adding rf)
weights = solve_weights(bl_excess + RISK_FREE, cov_returns.copy(), RISK_FREE)
mean, var = port_mean_var(weights, mean_returns.copy(), cov_returns.copy(),)
std = np.sqrt(var)
for name, fp in zip(names, weights):
    print('{} : {:.2f}% -> {} USD'.format(name, fp*100, round(fp*MONTOUSD, 2)))
